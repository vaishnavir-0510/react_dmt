// components/migration/tabs/ValidateTab.tsx
...
export const ValidateTab: React.FC = () => {
  ...
  const getActionButton = (field: CleanupField) => {
    ...
    if (field.change_log_id) {
      return (
        <Tooltip title={`Fix validation issue for ${field.field_name}`}>
          <Button
            variant="contained"
            color="warning"
            size="small"
    ...
    // Show Validate button for fields without issues
    return (
      <Tooltip title={`Run validation for ${field.field_name}`}>
        <Button
          variant="outlined"
          color="primary"
          size="small"
          startIcon={<ValidateIcon />}
          onClick={() => handleValidate(field.field_name)}
          sx={{ minWidth: '120px' }}
        >
          Validate
        </Button>
  ...
  if (!selectedObject) {
    return (
      <Box>
        ...
        <Alert severity="info">
          Please select an object to view validation data.
        </Alert>
      </Box>
    );
  }
  ...
  const fields = validateData?.changelog || [];
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" paragraph>
        Review and fix data validation issues for {selectedObject.object_name}. 
      ...
      {fields.length === 0 ? (
        <Paper elevation={1} sx={{ p: 3, mt: 2 }}>
          <Typography variant="body1" textAlign="center" color="text.secondary">
            No validation data found for this object.
          </Typography>
        </Paper>
      ) : (
