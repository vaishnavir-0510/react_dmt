// components/migration/tabs/TransformRuleTimelineSlideIn.tsx
...
import { useGetTransformRulesQuery, useApplyAllRulesMutation, useLazyGetCleanerTaskStatusQuery, useDeleteTransformRuleMutation } from '../../../services/transformRuleApi';

interface TransformRuleTimelineSlideInProps {
  open: boolean;
  onClose: () => void;
  objectId: string;
  objectName: string;
...
export const TransformRuleTimelineSlideIn: React.FC<TransformRuleTimelineSlideInProps> = ({
  open,
  onClose,
  objectId,
  objectName,
  ...
  const [reapplyingRules, setReapplyingRules] = useState<Set<number>>(new Set());
  const [reapplyResults, setReapplyResults] = useState<Record<number, any>>({});
  ...
  const [deleteTransformRule] = useDeleteTransformRuleMutation();
  ...
  const toggleCardExpansion = (ruleId: number) => {
  ...
  const pollReapplyStatus = async (ruleId: number, taskId: string) => {
    try {
      const result = await getCleanerTaskStatus({ taskId }).unwrap();
      ...
      if (result.status === 'SUCCESS' || result.status === 'FAILURE' || result.status === 'ERROR') {
        ...
        const ruleResult = result.result?.results?.find((r: any) => Number(r.rule_id) === ruleId);
  ...
  const handleDelete = async (rule: any) => {
    try {
      ...
      await deleteTransformRule({ ruleId: rule.rule_id }).unwrap();
        ...
        <Box sx={{
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h6" component="h2" fontWeight="bold" sx={{ fontSize: '1rem' }}>
              Transform Rule Timeline - {objectName}
            </Typography>
          </Box>
          <IconButton
            onClick={onClose}
          ...
          {isLoading ? (
            <Box display="flex" justifyContent="center" alignItems="center" minHeight="200px">
              ...
              <Typography variant="body2" sx={{ ml: 2 }}>
                Loading transform rules...
              </Typography>
            </Box>
          ) : error ? (
            <Alert severity="error">
              Failed to load transform rules. Please try again later.
            </Alert>
          ) : rules.length === 0 ? (
            <Alert severity="info">
              No transform rules found for this object.
            </Alert>
          ) : (
            <>
              <Box sx={{ mb: 2 }}>
                ...
                <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.75rem' }}>
                  Showing {rules.length} transform rule{rules.length !== 1 ? 's' : ''} executed on {objectName}
                </Typography>
              </Box>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {rules.map((rule, index) => {
                  ...
                  const sourceField = rule.params?.source_field || 'N/A';
                          ...
                          <Box sx={{ mt: 2, pt: 2, borderTop: 1, borderColor: 'divider' }}>
                            ...
                            <Box sx={{ mt: 2 }}>
                              ...
                              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                                {rule.params && Object.entries(rule.params).map(([key, value]) => (
                                  <Chip
                                    key={key}
                                    label={`${key}: ${value}`}
                                    size="small"
              ...
              <Box sx={{ mt: 3, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                ...
                />
                <Chip
                  label={`Failed: ${rules.filter(r => ['FAILURE', 'ERROR'].includes(r.status)).length}`}
                  variant="outlined"
