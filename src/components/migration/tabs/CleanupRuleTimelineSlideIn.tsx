// components/migration/tabs/CleanupRuleTimelineSlideIn.tsx
...
import { useGetCleanupRulesQuery, useApplyAllRulesMutation, useLazyGetCleanerTaskStatusQuery, useDeleteCleanerRuleMutation } from '../../../services/transformRuleApi';

interface CleanupRuleTimelineSlideInProps {
  open: boolean;
  onClose: () => void;
  objectId: string;
  objectName: string;
...
export const CleanupRuleTimelineSlideIn: React.FC<CleanupRuleTimelineSlideInProps> = ({
  open,
  onClose,
  objectId,
  objectName,
        ...
        <Box sx={{
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h6" component="h2" fontWeight="bold" sx={{ fontSize: '1rem' }}>
              Cleanup Rule Timeline - {objectName}
            </Typography>
          </Box>
          <IconButton
            onClick={onClose}
        ...
        <Box sx={{ flexGrow: 1, p: 3, overflow: 'auto' }}>
          {isLoading ? (
            ...
            <Alert severity="error">
              Failed to load cleanup rules. Please try again later.
            </Alert>
          ) : rules.length === 0 ? (
            <Alert severity="info">
              No cleanup rules found for this object.
            </Alert>
          ) : (
            <>
              <Box sx={{ mb: 2 }}>
                ...
                <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.75rem' }}>
                  Showing {rules.length} cleanup rule{rules.length !== 1 ? 's' : ''} executed on {objectName}
                </Typography>
              </Box>
